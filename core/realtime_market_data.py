...
class RealTimeMarketData:
    ...
    def bootstrap_window(self, 
        ...
        self.data_manager.gold_symbol = primary_symbol
        self.data_manager.silver_symbol = secondary_symbol
    ...
    def update_current_h1_bar(self, primary_price: float, secondary_price: float):
        ...
        Args:
            primary_price: Current gold price
            secondary_price: Current silver price
        """
        now = datetime.now()
    ...
    def calculate_hedge_quantities(self, 
                                   gold_lots: float) -> Tuple[float, float]:
        """
        Calculate hedge quantities for given gold position
        Uses current prices and volatility
        """
        if self.current_hedge_ratio is None:
            return (gold_lots, 0.0)
        ...
        # Calculate silver lots
        silver_lots = gold_lots * hedge_ratio
