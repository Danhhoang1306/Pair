...
class MT5Connector:
    ...
    def __init__(self, 
        ...
        Args:
            gold_symbol: Primary symbol (default: XAUUSD)
            silver_symbol: Secondary symbol (default: XAGUSD)
        ...
        self.gold_symbol = gold_symbol or 'XAUUSD'
        self.silver_symbol = silver_symbol or 'XAGUSD'
    ...
    def initialize(self, 
        ...
        try:
            ...
            _login = login or MT5_CONFIG['login']
            _password = password or MT5_CONFIG['password']
            _server = server or MT5_CONFIG['server']
            _path = path or MT5_CONFIG.get('path', '')
            
            # Initialize MT5
            if _path:
                if not mt5.initialize(path=_path, login=_login, password=_password, 
                                     server=_server, timeout=MT5_CONFIG['timeout']):
                    error = mt5.last_error()
                    logger.error(f"MT5 initialization failed: {error}")
                    return False
            else:
                if not mt5.initialize(login=_login, password=_password, 
                                     server=_server, timeout=MT5_CONFIG['timeout']):
                    error = mt5.last_error()
                    logger.error(f"MT5 initialization failed: {error}")
                    return False
            ...
            if not self._check_and_enable_symbols([self.gold_symbol, self.silver_symbol]):
                return False
    ...
    def get_current_prices(self, 
        ...
        try:
            ...
            primary_tick = mt5.symbol_info_tick(self.gold_symbol)
            secondary_tick = mt5.symbol_info_tick(self.silver_symbol)
    ...
    def get_pair_data(self,
        ...
        primary_df = self.get_bars(self.gold_symbol, timeframe, count, 
                               start_date, end_date, price_type)
        secondary_df = self.get_bars(self.silver_symbol, timeframe, count, 
                                 start_date, end_date, price_type)
        ...
        logger.info(f"Loaded pair data: Gold={len(primary_df)} bars, Silver={len(secondary_df)} bars")
    ...
    def get_ticks(self,
                  ...
                  start_date: datetime,
                  end_date: datetime = None,
                  flags: int = mt5.COPY_TICKS_ALL) -> Optional[pd.DataFrame]:
        """
    ...
    def __repr__(self):
        ...
        return f"MT5Connector(status={status}, gold={self.gold_symbol}, silver={self.silver_symbol})"
