...
class HedgeRatioCalculator:
    ...
    Example:
        ...
        >>> ratio = calc.calculate_optimal(gold_df, silver_df)
    ...
    def calculate_optimal(self,
        ...
        gold_prices = gold_df[price_col]
        silver_prices = silver_df[price_col]
        ...
        # OLS
        try:
            ols_result = self.calculate_ols(gold_prices, silver_prices)
        ...
        # Dollar-neutral
        try:
            dn_result = self.calculate_dollar_neutral(gold_prices, silver_prices)
        ...
        # Volatility-adjusted
        try:
            va_result = self.calculate_vol_adjusted(gold_prices, silver_prices)
        ...
        # Kalman
        try:
            kalman_result = self.calculate_kalman(gold_prices, silver_prices)
    ...
    def calculate_ols(self,
                     ...
                     silver_prices: pd.Series) -> HedgeRatioResult:
        """
        Calculate hedge ratio using OLS regression
        
        Gold = beta * Silver + alpha + residuals
        
        Args:
            gold_prices: Gold price series
            silver_prices: Silver price series
            
        Returns:
            HedgeRatioResult
        """
        # Prepare data
        y = gold_prices.values
        X = silver_prices.values
    ...
    def calculate_dollar_neutral(self,
        ...
        Ensures that $1 of gold is hedged with $1 of silver
        ...
        """
        # Simple price ratio
        ratio = gold_prices.iloc[-1] / silver_prices.iloc[-1]
        
        # Calculate spread with this ratio
        spread = gold_prices - ratio * silver_prices
        residual_std = spread.std()
        
        # R-squared approximation
        ss_res = np.sum(spread ** 2)
        ss_tot = np.sum((gold_prices - gold_prices.mean()) ** 2)
        r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
        ...
        result = HedgeRatioResult(
            ...
            metadata={
                ...
                'silver_price': silver_prices.iloc[-1]
            }
        )
    ...
    def calculate_vol_adjusted(self,
        ...
        Ratio = (Gold_Vol / Silver_Vol) * (Gold_Price / Silver_Price)
        ...
        silver_returns = silver_prices.pct_change().dropna()
        
        # Calculate rolling volatility
        gold_vol = gold_returns.rolling(window=lookback).std().iloc[-1]
        silver_vol = silver_returns.rolling(window=lookback).std().iloc[-1]
        
        # Base ratio (dollar neutral)
        base_ratio = gold_prices.iloc[-1] / silver_prices.iloc[-1]
        
        # Adjust for volatility
        vol_adjustment = gold_vol / silver_vol if silver_vol > 0 else 1.0
    ...
    def calculate_kalman(self,
        ...
        for i in range(len(gold_prices)):
            # Measurement
            y_obs = gold_prices.iloc[i]
            H = np.array([[silver_prices.iloc[i], 1.0]])  # Observation matrix
        ...
        # R-squared
        y_pred = ratio * silver_prices + x[1, 0]
        ss_res = np.sum((gold_prices - y_pred) ** 2)
        ss_tot = np.sum((gold_prices - gold_prices.mean()) ** 2)
        r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
    ...
    def calculate_rolling_ols(self,
        ...
        for i in range(window, len(gold_prices) + 1):
            gold_window = gold_prices.iloc[i-window:i]
            silver_window = silver_prices.iloc[i-window:i]
            
            result = self.calculate_ols(gold_window, silver_window)
            hedge_ratios.append(result.ratio)
        
        # Pad with NaN for first 'window' values
        hedge_ratios = [np.nan] * (window - 1) + hedge_ratios
        
        return pd.Series(hedge_ratios, index=gold_prices.index)
    
    def compare_methods(self,
                       gold_prices: pd.Series,
                       silver_prices: pd.Series) -> pd.DataFrame:
        """
        ...
        for method_name, method_func in methods:
            try:
                result = method_func(gold_prices, silver_prices)
                results.append({
                    'Method': method_name,
...
def quick_ols(gold_prices: pd.Series, silver_prices: pd.Series) -> float:
    """Quick OLS hedge ratio calculation"""
    calc = HedgeRatioCalculator()
    result = calc.calculate_ols(gold_prices, silver_prices)
    return result.ratio


def quick_optimal(gold_df: pd.DataFrame, silver_df: pd.DataFrame) -> float:
    """Quick optimal hedge ratio calculation"""
    calc = HedgeRatioCalculator()
    return calc.calculate_optimal(gold_df, silver_df)
