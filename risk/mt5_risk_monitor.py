...
class MT5RiskMonitor:
    ...
    def get_metrics(self,
        ...
        try:
            ...
            # Calculate hedge imbalance
            if target_hedge_ratio and abs(primary_net_lots) > 0:
                # Expected secondary lots based on target ratio
                expected_secondary_lots = abs(primary_net_lots) * target_hedge_ratio
                
                # Actual hedge ratio
                if abs(secondary_net_lots) > 0:
                    actual_hedge_ratio = abs(secondary_net_lots) / abs(primary_net_lots)
                else:
                    actual_hedge_ratio = 0.0
                
                # Imbalance in secondary lots
                # Positive = need more secondary, Negative = too much secondary
                hedge_imbalance = expected_secondary_lots - abs(secondary_net_lots)
                
                # Imbalance percentage
                if expected_secondary_lots > 0:
                    hedge_imbalance_pct = hedge_imbalance / expected_secondary_lots
                else:
