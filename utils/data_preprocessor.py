...
class DataPreprocessor:
    ...
    def preprocess_pair(self,
        ...
        Returns:
            Tuple of cleaned (gold_df, silver_df)
            
        Example:
            processor = DataPreprocessor()
            gold_clean, silver_clean = processor.preprocess_pair(gold_df, silver_df)
        """
        ...
        if fill_missing:
            gold_df = self.fill_missing_values(gold_df)
            silver_df = self.fill_missing_values(silver_df)
        
        # 2. Remove outliers
        if remove_outliers:
            gold_df, silver_df = self.remove_outliers_pair(gold_df, silver_df)
        
        # 3. Re-align after outlier removal
        gold_df, silver_df = self._align_dataframes(gold_df, silver_df)
        
        # 4. Add returns
        if add_returns:
            gold_df = self.add_returns(gold_df)
            silver_df = self.add_returns(silver_df)
        
        # 5. Sort by time
        gold_df = gold_df.sort_values('time').reset_index(drop=True)
        silver_df = silver_df.sort_values('time').reset_index(drop=True)
        
        # Store stats
        removed = original_len - len(gold_df)
        self.preprocessing_stats = {
            'original_length': original_len,
            'final_length': len(gold_df),
            'removed_bars': removed,
            'removal_rate': removed / original_len if original_len > 0 else 0
        }
        
        logger.info(f"[OK] Preprocessing complete: {original_len} -> {len(gold_df)} bars ({removed} removed)")
    ...
    def remove_outliers_pair(self,
                            gold_df: pd.DataFrame,
                            silver_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        gold_mask = self.detect_outliers(gold_df['close'])
        silver_mask = self.detect_outliers(silver_df['close'])
        
        # Combine masks (remove if outlier in either)
        combined_mask = ~(gold_mask | silver_mask)
        
        # Apply mask
        gold_df = gold_df[combined_mask].reset_index(drop=True)
        silver_df = silver_df[combined_mask].reset_index(drop=True)
