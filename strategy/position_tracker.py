...
class PositionTracker:
    ...
    def open_spread_position(self,
        ...
        """
        Open spread position (Gold + Silver)
        
        Args:
            gold_quantity: Gold quantity
            silver_quantity: Silver quantity
            gold_entry: Gold entry price
            silver_entry: Silver entry price
            side: 'LONG' or 'SHORT'
            hedge_ratio: Hedge ratio
            
        Returns:
            (gold_position, silver_position)
        """
        import uuid
        spread_id = str(uuid.uuid4())
        
        # Create Gold position
        gold_side = 'LONG' if side == 'LONG' else 'SHORT'
        gold_pos = self.open_position(
            symbol='XAUUSD',
            side=gold_side,
            quantity=gold_quantity,
            entry_price=gold_entry,
            metadata={'spread_id': spread_id, 'leg': 'gold', 'hedge_ratio': hedge_ratio}
        )
        ...
        # Link positions
        gold_pos.metadata['paired_position_id'] = silver_pos.position_id
        silver_pos.metadata['paired_position_id'] = gold_pos.position_id
        
        logger.info(f"Opened spread position: {side} (spread_id={spread_id[:8]})")
        
        return (gold_pos, silver_pos)
    ...
    def close_spread_position(self,
        ...
        # Try to find gold/silver or primary/secondary legs
        # Check metadata['leg'] for: 'gold', 'GOLD', 'XAU', 'primary', 'BTC'
        gold_pos = None
        silver_pos = None
        ...
        for p in spread_positions:
            ...
            # Check if this is primary/gold leg
            if any(x in leg or x in symbol for x in ['GOLD', 'XAU', 'BTC', 'PRIMARY']):
                gold_pos = p
            else:
                silver_pos = p
        
        # Fallback: if still not found, use first as gold
        if not gold_pos or not silver_pos:
            gold_pos = spread_positions[0]
            silver_pos = spread_positions[1]
        
        # Close both positions
        gold_pnl = self.close_position(gold_pos.position_id, gold_exit)
        silver_pnl = self.close_position(silver_pos.position_id, silver_exit)
        
        total_pnl = gold_pnl + silver_pnl
        
        logger.info(f"Closed spread {spread_id[:8]}: "
                   f"Gold PnL=${gold_pnl:.2f}, Silver PnL=${silver_pnl:.2f}, "
        ...
        return {
            ...
            'total_pnl': total_pnl,
            'gold_entry': gold_pos.entry_price,
            'gold_exit': gold_exit,
