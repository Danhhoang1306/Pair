...
class TradingSystem:
    ...
    def __init__(self, 
        ...
        self.primary_symbol = self.config.get('primary_symbol', 'XAUUSD')
        self.secondary_symbol = self.config.get('secondary_symbol', 'XAGUSD')
    ...
    def _recover_positions(self):
        ...
        4. If incomplete → Ask user (with timeout)
        5. If complete → Ask user to continue
        """
        logger.info("="*80)
        ...
        logger.warning("⚠️  Active setup flag detected - checking MT5 positions...")
        ...
        if not persisted_positions:
            logger.warning("⚠️  Setup flag active but no positions found - clearing flag")
        ...
        if len(mt5_tickets) == 0 and len(saved_tickets) > 0:
            logger.warning("="*80)
            logger.warning("⚠️  ALL POSITIONS CLOSED OFFLINE!")
            logger.warning("="*80)
        ...
        if missing_tickets:
            ...
            logger.error("="*80)
            logger.error("⚠️  INCOMPLETE SETUP - Some positions closed!")
            logger.error("="*80)
    ...
    def _rebalance_missing_positions(self, missing_tickets: set, persisted_positions: dict):
        ...
        for spread_id, legs in spreads.items():
            primary_pos = legs['gold']
            secondary_pos = legs['silver']
            ...
            if primary_missing or secondary_missing:
                ...
                if primary_missing:
                    logger.error(f"  ❌ Gold position missing (Ticket: {primary_pos.mt5_ticket})")
                    logger.error(f"  ✅ Silver position exists (Ticket: {secondary_pos.mt5_ticket})")
                    logger.error("  → CLOSING remaining silver position to avoid unhedged risk")
                    
                    # Close the remaining silver position
                    try:
                        result = self.trade_executor.close_position(secondary_pos.mt5_ticket)
                        if result.success:
                            logger.info(f"  ✅ Closed silver position {secondary_pos.mt5_ticket}")
                        else:
                            logger.error(f"  ❌ Failed to close silver: {result.comment}")
                    except Exception as e:
                        logger.error(f"  ❌ Error closing silver: {e}")
                
                else:  # secondary_missing
                    logger.error(f"  ✅ Gold position exists (Ticket: {primary_pos.mt5_ticket})")
                    logger.error(f"  ❌ Silver position missing (Ticket: {secondary_pos.mt5_ticket})")
                    logger.error("  → CLOSING remaining gold position to avoid unhedged risk")
                    
                    # Close the remaining gold position
                    try:
                        result = self.trade_executor.close_position(primary_pos.mt5_ticket)
                        if result.success:
                            logger.info(f"  ✅ Closed gold position {primary_pos.mt5_ticket}")
                        else:
                            logger.error(f"  ❌ Failed to close gold: {result.comment}")
                    except Exception as e:
                        logger.error(f"  ❌ Error closing gold: {e}")
        ...
        for spread_id, positions in spreads.items():
            try:
                ...
                if len(positions) == 2:
                    ...
                    # Use is_gold field to identify primary/secondary
                    primary_pos = positions[0] if positions[0].is_gold else positions[1]
                    ...
                    self.rebalancer.register_position(
                        ...
                        gold_lots=primary_pos.volume,
                        silver_lots=secondary_pos.volume,
                    ...
                    if mt5_pos and mt5_pos.comment:
                        ...
                        try:
                            ...
                            logger.info(f"✅ Registered {mt5_spread_id} in attribution engine (recovered)")
                        except Exception as e:
    ...
    def _handle_user_timeout(self):
        ...
        logger.error("❌ User timeout or declined - CLOSING ALL POSITIONS")
        self._emergency_close_all()
                ...
                try:
                    ...
                    if mt5_info:
                        ...
                        if balance_drift > 100:  # $100 drift warning
                            logger.warning(f"⚠️  Balance drift detected: "
                        ...
                        if dd_metrics.current_drawdown_pct > self.max_drawdown_limit:
                            logger.critical(f"🚨 REAL DRAWDOWN LIMIT EXCEEDED!")
        ...
        while self.running:
            try:
                ...
                with self.lock:
                    ...
                    # CRITICAL: Detect current position direction to prevent duplicate entries
                    if open_positions:
                        primary_positions = [p for p in open_positions if p.symbol == self.primary_symbol]
                        ...
                        if primary_positions:
                            ...
                            # If we have ANY LONG positions, we're in LONG mode
                            if long_positions:
                                current_position = 'LONG'
                                logger.debug(f"Current position: LONG ({len(long_positions)} positions)")
                            
                            # If we have ANY SHORT positions, we're in SHORT mode
                            elif short_positions:
                                current_position = 'SHORT'
                                logger.debug(f"Current position: SHORT ({len(short_positions)} positions)")
                            
                            # If we have BOTH (shouldn't happen in normal operation)
                            if long_positions and short_positions:
                                logger.warning(f"⚠️  Mixed positions detected: {len(long_positions)} LONG, {len(short_positions)} SHORT")
                    ...
                    if open_positions:
                        ...
                        # Queue pyramiding actions (grid-style position scaling)
                        for action in pyramid_actions:
                            ...
                            try:
                                self.signal_queue.put({
                                    'signal': signal,
                                    'data': data,
                                    'pyramiding': action  # Renamed from 'rebalance'
                                }, block=False)
                            except queue.Full:
                                logger.warning("Signal queue full")
                        
                        # Queue hedge adjustments (single-leg imbalance fixes)
                    ...
                    # Normal signal processing (entry/exit)
                    if signal.signal_type != SignalType.HOLD:
                        try:
                            self.signal_queue.put({
                                'signal': signal, 
                                'data': data,
                                'pyramiding': None  # Renamed from 'rebalance'
                            }, block=False)
                        except queue.Full:
                            logger.warning("Signal queue full")
    ...
    def _execution_thread(self):
        ...
        while self.running:
            try:
                ...
                pyramiding_action = signal_data.get('pyramiding')  # Renamed from 'rebalance'
                hedge_adjustment = signal_data.get('hedge_adjustment')
                
                with self.lock:
                    # Handle pyramiding (grid-style position scaling)
                    if pyramiding_action and pyramiding_action.get('type') == 'PYRAMIDING':
                        self._execute_pyramiding(pyramiding_action, data)
                        continue
                    
                    # Handle hedge adjustment (single-leg imbalance fix)
                    if hedge_adjustment:
                    ...
                    if is_entry:
                        logger.info(f"📥 Entry signal detected: {signal.signal_type.value}")
                        logger.info(f"   Z-score: {signal.zscore:.3f}")
                        logger.info(f"   Current positions: {len(self.position_tracker.positions)}")
                        logger.info(f"   Max positions: {self.max_positions}")
                    
                    # CRITICAL: Allow CLOSE signals even if max positions reached!
                    if is_exit:
                        # Always process exit signals
                        logger.info(f"📤 Exit signal: {signal.signal_type.value}")
                        self._execute_exit(signal, data)
                        logger.info(f"✅ Exit complete - positions now: {len(self.position_tracker.positions)}")
                        continue
                    ...
                    if not risk_result.passed:
                        ...
                        logger.warning(f"❌ Risk check FAILED - Entry blocked!")
                        logger.warning(f"   Failed checks: {failed_checks}")
                        logger.warning(f"   Signal: {signal.signal_type.value}, Z: {signal.zscore:.3f}")
                        logger.warning(f"   Position size: {pos_size_result.position_size:.2%}")
                        logger.warning(f"   Account balance: ${self.account_balance:,.2f}")
                        logger.warning(f"   Current drawdown: {dd_metrics.current_drawdown_pct:.2%}")
                        continue
    ...
    def _execute_entry(self, signal, data, pos_size_result):
        ...
        try:
            ...
            primary_quantity = primary_value / data['current_primary_price'] / 100
            ...
            logger.info(f"Position: Primary {primary_lots:.4f} lots, Secondary {secondary_lots:.4f} lots")
            ...
            (primary_result, secondary_result), spread_id, returned_zscore = self.trade_executor.place_spread_orders(
                gold_volume=primary_lots,
                silver_volume=secondary_lots,
                side=side,
            ...
            logger.info(f"  Primary: {primary_result.volume} lots @ ${primary_result.price:.2f} (Ticket {primary_result.order_ticket})")
            logger.info(f"  Secondary: {secondary_result.volume} lots @ ${secondary_result.price:.4f} (Ticket {secondary_result.order_ticket})")
            ...
            primary_pos.metadata['spread_id'] = spread_id  # Use ticket-based!
            secondary_pos.metadata['spread_id'] = spread_id  # Use ticket-based!
            ...
            primary_persisted = PersistedPosition(
                ...
                symbol=self.primary_symbol,  # Use configured symbol!
                side=primary_pos.side,
                volume=primary_pos.quantity,  # Position uses 'quantity' not 'volume'
                entry_price=primary_pos.entry_price,
                entry_time=str(primary_pos.opened_at),  # Convert datetime to string
                entry_zscore=entry_zscore,  # Use returned zscore
                hedge_ratio=snapshot.hedge_ratio,
                is_gold=True,  # Keep for backwards compatibility
            ...
            secondary_persisted = PersistedPosition(
                ...
                symbol=self.secondary_symbol,  # Use configured symbol!
                side=secondary_pos.side,
                volume=secondary_pos.quantity,  # Position uses 'quantity' not 'volume'
                entry_price=secondary_pos.entry_price,
                entry_time=str(secondary_pos.opened_at),  # Convert datetime to string
                entry_zscore=entry_zscore,  # Use returned zscore
                hedge_ratio=snapshot.hedge_ratio,
                is_gold=False,  # Keep for backwards compatibility
                created_at=datetime.now().isoformat(),
                last_updated=datetime.now().isoformat()
            )
            ...
            if spread_id:
                self.rebalancer.register_position(
                    ...
                    gold_lots=primary_result.volume,
                    silver_lots=secondary_result.volume,
    ...
    def _execute_pyramiding(self, rebalance, data):
        ...
        try:
            ...
            logger.info(f"  Secondary: +{secondary_lots:.4f} lots")
            
            # Execute orders with attribution tracking
            (primary_result, secondary_result), new_spread_id = self.trade_executor.place_spread_orders(
                gold_volume=primary_lots,
                silver_volume=secondary_lots,
                side=side,
                entry_zscore=snapshot.zscore
            ...
            logger.info(f"  Primary: {primary_result.volume} lots @ ${primary_result.price:.2f}")
            logger.info(f"  Secondary: {secondary_result.volume} lots @ ${secondary_result.price:.4f}")
    ...
    def _execute_hedge_adjustment(self, adjustment, data):
        ...
        This opens ONLY ONE LEG (the deficient side) to fix imbalance.
        Should result in ODD number of total MT5 positions.
        ...
        try:
            ...
            logger.info(f"  ⚠️  Opening SINGLE LEG only - position count will become ODD!")
            
            # Execute SINGLE order (not spread!)
            logger.info(f"📞 Calling place_market_order with:")
            logger.info(f"   symbol={adjustment.symbol}")
            logger.info(f"   order_type={'BUY' if adjustment.action == 'BUY' else 'SELL'}")
            logger.info(f"   volume={adjustment.quantity:.4f}")
            
            result = self.trade_executor.place_market_order(
                symbol=adjustment.symbol,
                order_type='BUY' if adjustment.action == 'BUY' else 'SELL',  # CORRECT ORDER!
                volume=adjustment.quantity,
                comment=f"HEDGE_ADJ:{spread_id[:8]}"
            )
            ...
            # CRITICAL: Verify position count
            import MetaTrader5 as mt5
            import time
            ...
            logger.info(f"📊 MT5 positions now: {count_after}")
            logger.info(f"   Is ODD? {count_after % 2 == 1}")
            
            if count_after % 2 == 0:
                logger.error("❌ CRITICAL: Position count is EVEN!")
                logger.error("   Hedge adjustment may have opened 2 legs!")
            ...
            if positions:
                ...
                if adjustment.action == 'BUY':
                    ...
                    position.quantity += result.volume
                    logger.info(f"  ✅ Updated {position.symbol} position: {position.quantity:.4f} lots")
                else:
                    # Reduce existing position
                    position.quantity -= result.volume
                    logger.info(f"  ✅ Reduced {position.symbol} position: {position.quantity:.4f} lots")
    ...
    def _execute_exit(self, signal, data):
        ...
        try:
            ...
            for spread_id in spread_ids:
                ...
                primary_pos = next((p for p in spread_positions if p.symbol == self.primary_symbol), None)
                secondary_pos = next((p for p in spread_positions if p.symbol == self.secondary_symbol), None)
                ...
                primary_result, secondary_result = self.trade_executor.close_spread_positions(
                    gold_ticket=primary_ticket,
                    silver_ticket=secondary_ticket
                )
                
                # Handle partial close failures
                primary_closed = primary_result.success
                secondary_closed = secondary_result.success
                
                if not primary_closed or not secondary_closed:
                    logger.error("="*80)
                    logger.error("⚠️  PARTIAL CLOSE FAILURE!")
                    logger.error("="*80)
                    
                    if not primary_closed:
                        logger.error(f"❌ Gold close FAILED: {primary_result.comment}")
                    else:
                        logger.info(f"✅ Gold closed: {primary_ticket}")
                    
                    if not secondary_closed:
                        logger.error(f"❌ Silver close FAILED: {secondary_result.comment}")
                    else:
                        logger.info(f"✅ Silver closed: {secondary_ticket}")
                    ...
                    if primary_closed:
                        ...
                        if primary_pos.position_id in self.mt5_tickets:
                            del self.mt5_tickets[primary_pos.position_id]
                        
                        # Remove gold from tracker
                        if primary_pos.position_id in self.position_tracker.positions:
                            del self.position_tracker.positions[primary_pos.position_id]
                        
                        # Archive gold position
                        self.persistence.delete_position(primary_pos.position_id)
                    ...
                    if secondary_closed:
                        ...
                        if secondary_pos.position_id in self.mt5_tickets:
                            del self.mt5_tickets[secondary_pos.position_id]
                        
                        # Remove silver from tracker
                        if secondary_pos.position_id in self.position_tracker.positions:
                            del self.position_tracker.positions[secondary_pos.position_id]
                        
                        # Archive silver position
                        self.persistence.delete_position(secondary_pos.position_id)
                    
                    # If one leg closed but other failed - DANGEROUS STATE!
                    if primary_closed != secondary_closed:
                        logger.error("="*80)
                        logger.error("🚨 CRITICAL: UNHEDGED POSITION!")
                        logger.error("="*80)
                        ...
                        remaining_ticket = secondary_ticket if primary_closed else primary_ticket
                        remaining_symbol = self.secondary_symbol if primary_closed else self.primary_symbol
                        ...
                        try:
                            ...
                            if retry_result.success:
                                logger.info(f"✅ Retry successful - {remaining_symbol} closed")
                                
                                # Cleanup the remaining position
                                if primary_closed:  # silver was remaining
                                    self.position_monitor.unregister_position(secondary_ticket)
                                    if secondary_pos.position_id in self.mt5_tickets:
                                        del self.mt5_tickets[secondary_pos.position_id]
                                    if secondary_pos.position_id in self.position_tracker.positions:
                                ...
                                logger.error(f"❌ Retry failed: {retry_result.comment}")
                    ...
                    if len(self.position_tracker.positions) == 0:
                        ...
                        logger.info("✅ Setup flag: INACTIVE (all positions eventually closed)")
                ...
                # CHECK IF ALL POSITIONS CLOSED → CLEAR FLAG
                if len(self.position_tracker.positions) == 0:
                    self.flag_manager.mark_setup_inactive("All positions closed")
                    logger.info("✅ Setup flag: INACTIVE (all positions closed)")
                    ...
                    if self.last_update_time:
                        try:
                            ...
                            for position in self.position_tracker.get_all_positions():
                                if position.symbol == self.primary_symbol:
                                    self.position_tracker.update_position_price(
                                        position.position_id,
                                        data['current_primary_price']
                                    )
                                elif position.symbol == self.secondary_symbol:
                                    self.position_tracker.update_position_price(
                                        position.position_id,
        ...
        while self.running:
            try:
                ...
                for pos in mt5_positions:
                    ...
                    if pos.symbol == self.primary_symbol:
                        leg = "PRIMARY"
                    elif pos.symbol == self.secondary_symbol:
                        leg = "SECONDARY"
                    else:
                        leg = "UNKNOWN"
                ...
                for spread_id, legs in spreads.items():
                        ...
                        if spread_id in attribution_engine.positions:
                            ...
                            if is_diverging:
                                ...
                                try:
                                    ...
                                    logger.critical(f"✅ Emergency closed spread {spread_id}")
                    ...
                    if components.hedge_quality < 0.7:
                        logger.warning(f"⚠️ HEDGE QUALITY LOW for {spread_id}!")
